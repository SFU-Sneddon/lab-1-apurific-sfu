...
```{bash, engine.opts='-l'}
#head -n 7 /local-scratch/course_files/MBB243/Sneddon_genotypes.txt
# translation:
...
# the latest genotype_head.txt file must have 50 lines (initially overwritten with 25 lines, appended with additional 25 lines)
...
# cut command took out the 4th column and overwrote genotype_cut.txt

#sort ~/MBB243/genotype_cut.txt > ~/MBB243/genotype_sort.txt
# sort command took out sorted the rows and overwrote genotype_sort.txt

#uniq ~/MBB243/genotype_sort.txt > ~/MBB243/genotype_uniq.txt
# uniq command identified the unique lines from a sorted txt file and overwrote genotype_uniq.txt

cut -f 4 ~/MBB243/genotype_head.txt | sort | uniq
# the cut command is similar to the output of the deconstructed code using the previous series of commands (i.e., similar to genotype_uniq.txt)
